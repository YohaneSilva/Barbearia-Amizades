...
def recuperarSenha(request):
    ...
    if request.method == 'POST': 
        ...
        if len(resultado_busca.values()) > 0:
            ...
            senha = geraSenha()
            data_local = dataLocal(' de ')
            Usuario.objects.filter(us_usuario=usuario).update(us_senha=senha)
            ...
            mensagem = """\
                ...
                """.format(especialista=especialista, data=data_local, senha=senha)

            envioDeEmail(assunto, mensagem, email)
        ...
        if len(reservas.values()) > 0:
            for index in range(len(reservas.values())):
                ...
                if nome_cliente == request.POST['nome-cliente']:
                    ...
                    return redirect('cadastrarAgendamentoHome')
...
def excluirAgendamento(request, id_registro):
    criarVariavelSessao(request)
    if request.session['logado'] == False:
        return redirect('acesso')
    ...
    data_agendada = resultado_busca.values('res_data_atendimento')[0]['res_data_atendimento']
    dia = data_agendada.strftime("%d")
    mes = data_agendada.strftime("%m")
    ano = data_agendada.strftime("%Y")

    data_agendada = '{dia} de {mes} de {ano}'.format(dia=dia, mes=meses_ano[mes], ano=ano)
...
def dataLocal(separador):
    ...
    data_local = '{dia}{separador}{mes}{separador}{ano}'.format(dia=dia_local, mes=mes_local, ano=ano_local, separador=separador)
